...
oddFactorial' n = case n of -1 -> 1
                            0 -> 1
                            n -> n * oddFactorial' (n - 2)
...
myGcd 0 n = n
myGcd n 0 = n
myGcd n k = if n > k then myGcd k (n - k) else myGcd n (k - n)
...
leftpad s p = if length s == p then s else leftpad (" " ++ s) p
...
smallestDivisor' n d = if n `mod` d == 0 then d else smallestDivisor' n (d + 1)
