...
firstHalf s = take half s where half = if (even . length) s 
                                        then midLength s
                                        else (midLength s) + (mod . length) s 2
                                        where midLength = (\x -> (div . length) x 2)
...
capitalize s = unwords [capitalize' w | w <- words s]
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) [k ^ x | x <- [0..max]]
...
joinToLength l xs = filter (\s -> (length s) == l) [x ++ y | x <- xs, y <- xs]
...
interpreter' [] _ = []
interpreter' (command:commands) (x:y:_)  = case result of
    (Left l) -> interpreter' commands [x1, y1]
    (Right r) -> r : (interpreter' commands [x1, y1])
    where result = interpreter'' command [x,y]
          formattedResult = either (const []) id result  
          x1 = either head (const x) result
          y1 = either (head . tail) (const y) result

interpreter'' :: String -> [Int] -> Either [Int] String
interpreter'' command (x:y:_) = case command of
    "up" -> Left [x, y + 1]
    "down" -> Left [x, y - 1]
    "left" -> Left [x - 1, y]
    "right" -> Left [x + 1, y]
