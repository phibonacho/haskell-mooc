...
treeMax Empty = 0
treeMax (Node v l r)
  | lMax >= v && lMax >= rMax = lMax
  | rMax >= v && rMax >= lMax = rMax
  | otherwise =  v
  where
    lMax = treeMax l
    rMax = treeMax r
...
walk (s:steps) (Node v l r) = case s of
  StepL -> walk steps l
  StepR -> walk steps r
...
set (p:path) val (Node v l r) = case p of
  StepL -> Node v (set path val l) r
  StepR -> Node v l (set path val r)
...
search val (Node v l r)
  | v == val = Just []
  | lSearch /= Nothing = Just (StepL:fromJust lSearch)
  | rSearch /= Nothing = Just (StepR:fromJust rSearch)
  | otherwise = Nothing
  where
    lSearch = search val l
    rSearch = search val r
