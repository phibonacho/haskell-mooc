...
allEqual [] = True
allEqual (x:xs) = foldr f True xs where f y y1 = y == x && y1 -- this could be achieved in an easier way lol
...
middle x y z = r where q:r:s = sort [x, y, z] -- sort [x,y,x] !! 1 works better
...
rangeOf :: (Ord a, Foldable t, Num a) => t a -> a
rangeOf s = maximum s - minimum s
...
longest :: (Ord a) => [[a]] -> [a]
longest [] = []
longest s = minimumBy (comparing head) $ filter(\x -> length x == length longestList) s
    where longestList = maximumBy (comparing length) s
...
incrementKey k = map (\(x, xv) -> if x == k then (x, xv + 1) else (x, xv))
...
winner scores player1 player2 = if p1 >= p2 then player1 else player2
    where p1:p2:_ = map (\x -> Map.findWithDefault 0 x scores) [player1, player2]
...
transfer from to amount bank = if Map.member from bank && Map.member to bank && amount >= 0 && fromDelta >= 0
    then
        transfer' to amount $ transfer' from (negate amount) bank
    else
        bank
    where
        fromDelta = Map.findWithDefault 0 from bank - amount

transfer' :: String -> Int -> Map.Map String Int -> Map.Map String Int
transfer' to amount = Map.alter transaction to
    where
        transaction :: Maybe Int -> Maybe Int
        transaction (Just credit) = Just $ credit + amount
        transaction Nothing = Just 0
...
maxIndex arr = fst $ maximumBy (comparing f) (assocs arr) where f (i, v) = v
