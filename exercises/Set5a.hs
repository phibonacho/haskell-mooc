...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Invert color) = [1 - r color, 1 - g color, 1 - b color]
  where
      r :: Color -> Double
      r color = head $ rgb color
      g :: Color -> Double
      g color = rgb color !! 1
      b :: Color -> Double
      b color = rgb color !! 2
...
fromList = foldr (\(k, v) prev -> Pair k v prev) Empty
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne one) = 1 + fromNat one
...
fromBin' :: Int -> Bin -> Int
fromBin' _ End = 0
fromBin' base (I b) = 2^base + fromBin' (base + 1) b
fromBin' base (O b) = fromBin' (base + 1) b
